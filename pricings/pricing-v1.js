...
const pricingV1 = (body) => {
  ...
  const endHour = (new Date(Date.parse(body.startTime) + body.duration * 1000)).getHours();

  const temp = new Date(body.startTime);
  const temp2 = new Date(Date.parse(body.startTime) + body.duration * 1000);
